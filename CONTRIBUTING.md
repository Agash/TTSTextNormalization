# Contributing to TTSTextNormalization
...
```bash
dotnet restore TTSTextNormalization.sln
dotnet build TTSTextNormalization.sln
dotnet test TTSTextNormalization.sln
```
...
```bash
dotnet test TTSTextNormalization.sln --filter "TestCategory!=Integration"
```
